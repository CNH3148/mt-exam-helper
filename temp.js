...
async function init() {
    ...
    bcSubject.onclick = () => {
        const targetSub = bcSubject.textContent;
        if (targetSub) {
            filterSubject.value = targetSub;
        }
        switchGlobalMode('general');
    };
    bcTopic.onclick = () => {
        const targetSub = bcSubject.textContent;
        if (targetSub) {
    ...
    if (globalMode === 'general') {
        ...
        if (globalMode === 'wrong') {
            ...
            multiData = multiData.filter(q => {
                ...
                return state.current_answer !== q.answer && !state.is_fixed;
            });
...
function renderCardView() {
    ...
    if (starBtn) {
        ...
        starBtn.className = isBookmarked ? 'star-icon active' : 'star-icon';
        ...
        card.innerHTML = `
            ...
            <div class="list-card-main">
                ...
                <div class="list-explanation" id="list-exp-${idx}" style="display:none; margin-top:12px; padding:12px; border:1px solid var(--glass-border); border-radius:8px;">
                    ...
                    <div id="md-preview-list-${q.exam_id}-${q.no}" class="markdown-body" style="background:var(--bg-lighter); padding:12px; border-radius:6px; min-height:60px;">${marked.parse(getExplanationState(q) || q.explanation || '*尚無筆記*')}</div>
                    <div id="md-editor-list-${q.exam_id}-${q.no}" style="display:none; margin-top:8px;">
                        <textarea id="user-exp-list-${q.exam_id}-${q.no}" style="width:100%; height:80px; background:var(--bg-lighter); color:var(--text-main); border:1px solid var(--glass-border); padding:8px; border-radius:6px; resize:vertical;">${getExplanationState(q) || q.explanation || ''}</textarea>
                        <div style="text-align:right; margin-top:8px;">
