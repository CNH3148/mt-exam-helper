                    ...
                    try {
                        await fetch('/api/add_tag_batch', {
                            ...
                            body: JSON.stringify({
                                ...
                                questions: [{exam_id: targetQ.exam_id, no: targetQ.no, year: targetQ.year}],
                                tag: selection
                            })
                        });
...
async function applyFilters() {
    ...
    if (globalMode === 'general') {
        ...
        filteredData = currentData.filter(q => {
            return (q.year && checkedYears.includes(q.year)) || checkedYears.length === 0;
        });
...
window.togglePdfView = function(idx = null) {
    ...
    if (!isActive) {
        ...
        const subject = currentActiveSubject;
        const year = q.year;
        let searchText = q.question.replace(/<[^>]*>?/gm, '').trim();
        let match = searchText.match(/[\w一-龥]{6,}/);
        if (match) {
            searchText = match[0];
        } else {
            searchText = searchText.substring(0, 10);
        }
        const url = `/pdfs/${encodeURIComponent(subject)}/${year}.pdf#navpanes=0&view=FitH&search=${encodeURIComponent(searchText)}`;
        pdfIframe.src = url;
    } else {
...
function renderCardView() {
    isPdfViewActive = false;
    document.getElementById('q-options').classList.remove('pdf-mode');
    ...
    if (q.tags) {
        q.tags.forEach(tag => {
            ...
            span.innerHTML = `<span class="tag-text">${tag}</span><svg class="eye-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>`;
            qTags.appendChild(span);
        });
    }
    
    let safeQuestion = q.question.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    let htmlContent = `${q.no}. ${safeQuestion}`;
    ...
    q.choices.forEach((choiceText, i) => {
        ...
        const safeChoiceText = choiceText.replace(/</g, '&lt;').replace(/>/g, '&gt;');
        div.innerHTML = `<span class="option-letter">${letter}.</span> <span class="option-text">${safeChoiceText}</span>`;
        ...
        qExplanation.innerHTML = `
            ...
            <div style="margin-top:8px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                    <span style="font-size:0.9em; font-weight:bold; color:var(--primary);">📝 我的筆記</span>
...
function renderListView() {
    ...
    currentActiveTopicData.forEach((q, idx) => {
        ...
        let questionTagsHtml = q.tags ? q.tags.map(t => `<span class="tag native-tag" onclick="togglePdfView(${idx})" title="點擊查看原始 PDF" style="background:transparent; border:1px solid #ddd; color:#888;"><span class="tag-text">${t}</span><svg class="eye-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></span>`).join('') : '';
